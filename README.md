# The Opera House - [Allegro](https://cse110-sp26-group1.github.io/cse110-sp26-group1/html/login.html)
Team-based issue tracker for developers who want AI-assisted issue creation
without giving up complete control of the workflow to agents. Humans manage issues in the web UI and can use their agents to work on and update those issues through our CLI tool.


## Documentation

| Guide | Description |
|-------|-------------|
| [Product Overview](docs/architecture/overview.md) | Architecture, teams, issues, agents |
| [User Guide](docs/user-guide/README.md) | Step-by-step workflows (web & CLI) |
| [API Reference](docs/api/issues.md) | HTTP endpoints |
| [Contributing](CONTRIBUTING.md) | Setup, branches, tests, PRs |

## Quick links

- [Backend setup](docs/backend/setup.md)
- [CLI user guide](cli/CLT-user-guide.md)

## Team Details
- [Team Page](admin/team.md)
- [Team Status Video 1](https://www.youtube.com/watch?v=6gJ4FE0Xw8E)
- [Allegro](https://cse110-sp26-group1.github.io/cse110-sp26-group1/html/login.html)
