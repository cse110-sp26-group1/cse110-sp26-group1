...
/**
 * Creates a test user and returns their ID and auth token
 * @param {string} username - User's username
 * @param {string} email - User's email address
 * @returns {Promise<{id: number, token: string}>} User ID and auth token
 */
...
/**
 * Creates a test team using the API endpoint
 * @param {string} token - Auth token of team creator
...
async function makeAdmin(teamId, userId) {
	await env.DB.prepare('UPDATE team_members SET role = ? WHERE team_id = ? AND user_id = ?').bind('admin', teamId, userId).run();
}

/**
 * Returns authorization headers for API requests
 * @param {string} token - Bearer token
 * @returns {{Authorization: string, 'Content-Type': string}} Headers object
 */
...
/**
 * Test suite for invites endpoints
 */
describe('Invites Endpoints', () => {
	...
	describe('PATCH /invites/:id/accept', () => {
		it('should accept invite and add user to team', async () => {
			...
			const memberCheck = await env.DB.prepare('SELECT * FROM team_members WHERE team_id = ? AND user_id = ?')
				.bind(teamId, invitee.id)
				.first();
	...
	describe('PATCH /invites/:id/reject', () => {
		it('should decline invite without adding to team', async () => {
			...
			const inviteCheck = await env.DB.prepare('SELECT status FROM invites WHERE id = ?').bind(invite_id).first();
	...
	describe('DELETE /invites/:id', () => {
		it('should allow inviter to delete invite', async () => {
			...
			const checkInvite = await env.DB.prepare('SELECT * FROM invites WHERE id = ?').bind(invite_id).first();
