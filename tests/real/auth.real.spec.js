// @ts-check
// Real-backend version of auth.spec.js. Talks to the live Worker via
// tests/helpers/real-api.js. Run with `npm run test:e2e:real`.
import { test, expect } from '@playwright/test';
import { configureRealApiPage, makeUniqueUser, registerRealUser, setBrowserSession } from '../helpers/real-api.js';

test.describe('Auth — login page (real backend)', () => {
	test('redirects to teams when already signed in', async ({ page }) => {
		const session = await registerRealUser(makeUniqueUser('login_redirect'));
	...
	test('rejects invalid credentials and stays on login.html', async ({ page }) => {
		const session = await registerRealUser(makeUniqueUser('bad_login'));
		await configureRealApiPage(page);
	...
	test('successful login stores token and lands on teams.html', async ({ page }) => {
		const session = await registerRealUser(makeUniqueUser('good_login'));
		await configureRealApiPage(page);
	...
	test('after login, returns user to the ?redirect= page they came from', async ({ page }) => {
		const session = await registerRealUser(makeUniqueUser('login_redir_param'));
		await configureRealApiPage(page);
...
test.describe('Auth — signup page (real backend)', () => {
	...
	test('duplicate account (HTTP 409) surfaces a friendly error and never the generic "Sign-up failed" fallback', async ({ page }) => {
		...
		const existing = await registerRealUser(makeUniqueUser('signup_dup'));
		await configureRealApiPage(page);
...
test.describe('Auth gating (real backend)', () => {
	...
	test('sign-out from the user menu clears storage and returns to login', async ({ page }) => {
		const session = await registerRealUser(makeUniqueUser('signout'));
		await setBrowserSession(page, session);
