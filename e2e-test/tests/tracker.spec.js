...
test.describe('Tracker Page Features', () => {
	test('download skills.md button downloads SKILL.md file', async ({ page }) => {
		...
		try {
			...
			await expect(page.locator('#toast')).toContainText('downloaded', { timeout: 5000 });
		} finally {
...
Stack: at handleConnection (/app/src/db.js:42:15)`;
		...
		try {
			...
			const issueTitle = `Issue with log upload ${Date.now()}`;
			await page.fill('#new-title', issueTitle);
			...
			await expect(page.locator('#toast')).toContainText('Tagged as a', { timeout: 10000 });

			// Wait for the issue list to have the new issue using .issue-row selector
			await expect(page.locator('#issue-list')).toContainText(issueTitle, { timeout: 15000 });

			// Verify via API that issue was created
			const issues = await fetchIssues(owner, team.id);
			const createdIssue = issues.find((i) => i.title === issueTitle);
			expect(createdIssue).toBeDefined();
			expect(createdIssue.description).toContain('Testing file upload via dropzone');
		} finally {
			await safeDeleteTeam(owner, team.id);
		}
	});
	...
	test('dropzone rejects non-text files', async ({ page }) => {
		...
		try {
			...
			await expect(page.locator('#toast')).toContainText('Only text/log files are allowed', { timeout: 5000 });

			await expect(page.locator('#file-list .file-name')).toHaveCount(0);
			await expect(page.locator('#new-backdrop')).toBeVisible();
		} finally {
			await safeDeleteTeam(owner, team.id);
		}
	});
